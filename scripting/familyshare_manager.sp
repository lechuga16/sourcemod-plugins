...
public OnPluginStart()
{
    ...
    g_hCvar_AppId = CreateConVar("sm_familyshare_appid", "550", "Application ID of current game. HL2:DM (320), CS:S (240), CS:GO (730), TF2 (440)");
    g_hCvar_APIKey = CreateConVar("sm_familyshare_apikey", "XXXXXXXXXXXXXXXXXXXX", "Steam developer web API key", FCVAR_PROTECTED);
    g_hCvar_BanMessage = CreateConVar("sm_familyshare_banmessage", "Family sharing is disabled.", "Message to display in sourcebans/on ban");
    g_hCvar_IgnoreAdmins = CreateConVar("sm_familyshare_ignoreadmins", "1", "Check and unblock admins?", FCVAR_NONE, true, 0.0, true, 1.0);
    ...
    g_hCvar_ban = CreateConVar("sm_familyshare_ban", "0", "the time that will be banned", FCVAR_NONE, true, 0.0);
...
public Action:command_removeFromList(client, args)
{
    ...
    if(hFile == INVALID_HANDLE)
    {
        ...
        CPrintToChat(client, "{blue}[{default}Family Share Manager{blue}]{default} Plugin has encountered a critial error with the list file.");
        CloseHandle(hFile);
    ...
    if(args == 0)
    {
        CPrintToChat(client, "{blue}[{default}Family Share Manager{blue}]{default} Invalid Syntax: sm_removefromlist <steam id>");
        return Plugin_Handled;
    }
    ...
    while(!IsEndOfFile(hFile) && ReadFileLine(hFile, steamid, sizeof(steamid)))
    {
        ...
        CPrintToChat(client, "{blue}[{default}Family Share Manager{blue}]{default} {olive}%s{default} - {olive}%s{default}", steamid, playerSteam);
    ...
    if(found)
    {
        ...
        CPrintToChat(client, "{blue}[{default}Family Share Manager{blue}]{default} Found Steam ID: {olive}%s{default}, removing from list...", playerSteam);
	...
	else CPrintToChat(client, "{blue}[{default}Family Share Manager{blue}]{default} Steam ID: {olive}%s{default} not found, no action taken.", playerSteam);
    return Plugin_Handled;
}
...
public Action:command_addToList(client, args)
{
    ...
    switch(args)
    {
        ...
        case 0:
        {
            ...
            CPrintToChat(client, "{blue}[{default}Family Share Manager{blue}]{default} Displaying players menu...");
        ...
        default:
        {
            ...
            {
                CPrintToChat(client, "{blue}[{default}Family Share Manager{blue}]{default} Invalid Input - Not a Steam 2 ID. (STEAM_0:X:XXXX)");
                CloseHandle(hFile);
                return Plugin_Handled;
            }
            ...
            CPrintToChat(client, "{blue}[{default}Family Share Manager{blue}]{default} Successfully added {olive}%s{default} to the list.", steamid);
            CloseHandle(hFile);
            parseList();
            return Plugin_Handled;
        }
...
public playerMenuHandle(Handle:playerMenu, MenuAction:action, client, menuItem)
{
    ...
    {   
        ...
        WriteFileLine(hFile, steamid);
        CPrintToChat(client, "{blue}[{default}Family Share Manager{blue}]{default} Successfully added {olive}%s (%N){default} to the list.", steamid, target);
        CloseHandle(hFile);
        parseList();
        return;
    }
...
public Action:command_displayList(client, args)
{
    ...
    while(!IsEndOfFile(hFile) && ReadFileLine(hFile, auth, sizeof(auth)))
    {
        ...
        if(StrContains(auth, "STEAM_", false) != -1)
        {
            ...
            CPrintToChat(client, "{blue}[{default}Family Share Manager{blue}]{default} {olive}%s{default}", auth);
        }
    }
...
{
    CPrintToChat(client, "{blue}[{default}Family Share Manager{blue}]{default} Rebuilding whitelist...");
...
parseList(bool:rebuild = false, client = 0)
{
    ...
    if(rebuild && client) CPrintToChat(client, "{blue}[{default}Family Share Manager{blue}]{default} Rebuild complete!");
    g_bParsed = true;
    CloseHandle(hFile);
}
...
public OnSocketDisconnected(Handle:socket, any:userid)
{
    ...
    if (client > 0)
    {
        ...
        else
        {
            ...
            {
                LogMessage("Banning %L for %i minutes", client,GetConVarInt(g_hCvar_ban));
                ServerCommand("sm_ban #%i %i \"%s\"", userid,GetConVarInt(g_hCvar_ban), banMessage);
            }
        }
    }
