...
l4d_ready_secret, l4d_ready_afk, cvarGameMode, cvarScavRestart;
...
public void OnPluginStart()
{
	...
	l4d_ready_afk 				= CreateConVar("l4d_ready_afk", "180", "Send AFK player to spectator", _, true, 1.000);
...
stock void PlayerAfkGoSpec(int client, float fTime)
{
	if (fTime - g_fButtonTime[client] > l4d_ready_afk.FloatValue){
		ChangeClientTeam(client, L4D2Team_Spectator);
		CPrintToChatAll("{blue}[{olive}!{blue}]{default} Player {olive}%N{default} has moved to spectator for AFK.",client);
	}
