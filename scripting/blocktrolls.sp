...
public OnPluginStart()
{
	...
	timewait = CreateConVar("sm_votetime", "40.0", "waiting time to start voting");
}
...
public OnMapStart()
{
	...
	loadedPlayers = 0;
	CreateTimer(GetConVarFloat(timewait), EnableCallvoteTimer);
}
...
public Action:Vote_Listener(client, const String:command[], argc)
{
	if (g_bBlockCallvote)
	{
	CPrintToChat(client, "{blue}[{default}SM{blue}] {default}Voting is not enabled until{olive} %i{default}s into the round",GetConVarInt(timewait));
