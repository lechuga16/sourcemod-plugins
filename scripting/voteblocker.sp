...
Handle g_hEnable;
Handle g_hLog;
Handle g_hRespectLevels;
Handle g_hBlockCount;
Handle g_hAdminImmunity;
Handle g_hTankImmunity;
Handle g_hSpectatorVote;
Handle g_hKick;
Handle g_hReturnToLobby;
Handle g_hChangeAlltalk;
Handle g_hRestartGame;
Handle g_hChangeMission;
Handle g_hChangeChapter;
Handle g_hChangeDifficulty;

// Set up strings for tracking issues and targets etc. of each client
char g_sIssue[MAXPLAYERS+1][32], g_sTarget[MAXPLAYERS+1][32], g_sCaller[MAXPLAYERS+1][32], g_sCallerAuth[MAXPLAYERS+1][32], g_sCallerName[MAXPLAYERS+1][32];

// Set up integer for tracking block count of each client
int g_iBlockCount[MAXPLAYERS+1] = 0;
...
public void OnPluginStart()
{
	...
	g_hLog				= CreateConVar("vb_log",				"1",	"0 - Disable logging of blocked votes, 1 - Enable logging of blocked votes",													0, true, 0.0, true, 1.0);
	...
	g_hSpectatorVote	= CreateConVar("vb_spectatorvote",		"0",	"0 - Allow this type of vote, x - Only clients that match one or more of these flags can call this vote",						0, true, 0.0, true, 0.0);
	g_hKick				= CreateConVar("vb_kick",				"b",	"0 - Allow this type of vote, x - Only clients that match one or more of these flags can call this vote",						0, true, 0.0, true, 0.0);
	g_hReturnToLobby	= CreateConVar("vb_returntolobby",		"0",	"0 - Allow this type of vote, x - Only clients that match one or more of these flags can call this vote",						0, true, 0.0, true, 0.0);
	g_hChangeAlltalk	= CreateConVar("vb_changealltalk",		"0",	"0 - Allow this type of vote, x - Only clients that match one or more of these flags can call this vote",						0, true, 0.0, true, 0.0);
...
public Action Listener_CallVote(int client, const char[] command,int argc)
{
	if (GetConVarInt(g_hEnable) == 1)
	{
		...
		GetClientAuthId(client, AuthId_Engine, g_sCallerAuth[client], sizeof(g_sCallerAuth[]), true);
		GetClientName(client, g_sCallerName[client], sizeof(g_sCallerName[]));
		int target = GetClientOfUserId(StringToInt(g_sTarget[client]));
		...
		if (StrEqual(g_sIssue[client], "kick", false))
		{
			...
			AdminId callerid = GetUserAdmin(client);
...
bool FlagMatch(int client, const char[] flagString)
{
	...
	{
		int found, flags = ReadFlagString(flagString);
...
bool IsClientAuthor(int client)
{
	...
	GetClientAuthId(client, AuthId_Engine, sAuth, 24, true);
